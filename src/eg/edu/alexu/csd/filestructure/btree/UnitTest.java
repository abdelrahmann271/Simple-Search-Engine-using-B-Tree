...
public class UnitTest {
    ...
    public void testindexWebPage() {
        ...
        try {
            ...
            List<ISearchResult> expected = Arrays.asList(new SearchResult[]{new SearchResult("7700521", 1)});
            List<ISearchResult> actual = searchEngine.searchByWordWithRanking("zbor");
    ...
    public void testindexWebDirectoryComplex() {
         ...
         */
        try {
            searchEngine.indexDirectory("res\\multiplewordwithranking");
            List<ISearchResult> expected = Arrays.asList(new SearchResult[]{new SearchResult("7734119", 1)});
            List<ISearchResult> actual = searchEngine.searchByMultipleWordWithRanking("CoRomaNdel PenInSuLa ZeAlAnD"); //res\\multiplewordwithranking\\subfolder1
            Assert.assertEquals(expected.size(), actual.size());
            Comparator<ISearchResult> comp = new Comparator<ISearchResult>() {
                @Override
                public int compare(ISearchResult o1, ISearchResult o2) {
            ...
            for (int i = 0; i < expected.size(); i++) {
                Assert.assertEquals(actual.get(i).getRank(), expected.get(i).getRank());
                Assert.assertEquals(actual.get(i).getId(), expected.get(i).getId());
            }
            expected = Arrays.asList(new SearchResult[]{new SearchResult("7734133", 2), new SearchResult("7742161", 3)
                    , new SearchResult("7738278", 8), new SearchResult("7734140", 4)
            });
            actual = searchEngine.searchByMultipleWordWithRanking("OmaR Nour"); //scattered through wiki08-09-10
            Assert.assertEquals(expected.size(), actual.size());
            Collections.sort(actual, comp);
            Collections.sort(expected, comp);
            for (int i = 0; i < expected.size(); i++) {
            ...
            actual = searchEngine.searchByMultipleWordWithRanking("Coromandel Ariadaeus"); //doesn't exist in one doc
